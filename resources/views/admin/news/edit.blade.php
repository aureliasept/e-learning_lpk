...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <nav class="flex items-center space-x-2 text-sm mb-6">
        ...
        <span class="text-[#d4af37] font-medium">Edit Berita</span>
    </nav>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white tracking-wide">Edit Berita</h1>
                <p class="text-sm text-gray-400">Perbarui konten berita</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-b from-[#0f172a] to-[#0b1221] border border-[#1e293b] rounded-2xl shadow-2xl overflow-hidden">
        <form action="{{ route('admin.news.update', $news->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-8 space-y-6">
                ...
                <div>
                    <label class="block text-[#d4af37] text-xs font-bold uppercase mb-2 tracking-wider">Gambar Saat Ini</label>
                    @if($news->image)
                        <div class="bg-[#0b1221] rounded-xl p-4 border border-[#1e293b] mb-4">
                            <img src="{{ $news->image_url }}" alt="{{ $news->title }}" class="h-32 rounded-lg object-cover">
                        </div>
                    @else
                        <p class="text-sm text-gray-500 mb-4">Tidak ada gambar.</p>
                    @endif

                    <label class="block text-[#d4af37] text-xs font-bold uppercase mb-2 tracking-wider">Ganti Gambar</label>
                ...
                <div>
                    ...
                    <textarea name="content" rows="10" required
                        class="w-full bg-[#0b1221] border border-[#1e293b] text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#d4af37]/50 focus:border-[#d4af37] transition-all resize-none">{{ old('content', $news->content) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full bg-[#0b1221] border border-[#1e293b] text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#d4af37]/50 focus:border-[#d4af37] transition-all appearance-none">
                        ...
                        <option value="publish" {{ old('status', $news->status ?? 'draft') === 'publish' ? 'selected' : '' }}>Publish</option>
                    </select>
                </div>
