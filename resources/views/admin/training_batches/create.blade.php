...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <nav class="flex items-center space-x-2 text-sm mb-6">
        ...
        @endif
        <span class="text-[#d4af37] font-medium">Tambah Gelombang</span>
    </nav>
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white tracking-wide">Tambah Gelombang</h1>
                <p class="text-sm text-gray-400">Buat gelombang pelatihan baru</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-b from-[#0f172a] to-[#0b1221] border border-[#1e293b] rounded-2xl shadow-2xl overflow-hidden">
        <form action="{{ route('admin.training_batches.store') }}" method="POST">
            @csrf
            ...
            <div class="p-8 space-y-6">
                <div>
                    ...
                    <select name="training_year_id" required
                        ...
                        @foreach($years as $y)
                            <option value="{{ $y->id }}" {{ old('training_year_id', $year?->id) == $y->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Gelombang 1"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="start_date_picker" name="start_date" value="{{ old('start_date') }}" required placeholder="dd/mm/yyyy"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="end_date_picker" name="end_date" value="{{ old('end_date') }}" required placeholder="dd/mm/yyyy"
                                class="w-full bg-[#0b1221] border border-[#1e293b] text-white rounded-xl px-4 py-3 pr-12 focus:ring-2 focus:ring-[#d4af37]/50 focus:border-[#d4af37] placeholder-gray-600 transition-all">
                            <div class="absolute inset-y-0 right-0 flex items-center pr-4 pointer-events-none">
                                <svg class="w-5 h-5 text-[#d4af37]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                ...
                <div class="flex flex-col-reverse sm:flex-row justify-end gap-4 border-t border-[#1e293b] pt-6">
                    <a href="{{ $year ? route('admin.training_years.show', $year->id) : route('admin.training_years.index') }}" 
