...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <nav class="flex items-center space-x-2 text-sm mb-6">
        ...
        <span class="text-[#d4af37] font-medium">Edit Tahun {{ $year->name }}</span>
    </nav>
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-white tracking-wide">Edit Tahun {{ $year->name }}</h1>
                <p class="text-sm text-gray-400">Perbarui data tahun pelatihan</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-b from-[#0f172a] to-[#0b1221] border border-[#1e293b] rounded-2xl shadow-2xl overflow-hidden">
        <form action="{{ route('admin.training_years.update', $year->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-8 space-y-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $year->name) }}" required placeholder="Contoh: 2025"
                ...
                <div>
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $year->is_active) ? 'checked' : '' }}
