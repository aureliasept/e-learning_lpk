{{-- 
    Komponen Modal Konfirmasi Hapus yang Reusable
    Gunakan dengan Alpine.js
    
    Contoh Penggunaan:
    <x-delete-modal 
        id="delete-instructor-{{ $instructor->id }}"
        title="Hapus Instruktur"
        message="Apakah Anda yakin ingin menghapus data instruktur ini? Tindakan ini tidak dapat dibatalkan."
        :action="route('admin.instructors.destroy', $instructor->id)"
    />
    
    Untuk trigger modal, gunakan:
    <button @click="$dispatch('open-delete-modal', 'delete-instructor-{{ $instructor->id }}')">Hapus</button>
--}}
...
<div 
    x-data="{ open: false }"
    x-show="open"
    x-on:open-delete-modal.window="if ($event.detail === '{{ $id }}') open = true"
        ...
        <div class="flex min-h-full items-center justify-center p-4">
            ...
            >
                ...
                <div class="relative p-6">
                    ...
                    {{-- Judul --}}
                    <h3 class="text-xl font-bold text-white text-center mb-3" id="modal-title-{{ $id }}">
                        {{ $title }}
                    </h3>
