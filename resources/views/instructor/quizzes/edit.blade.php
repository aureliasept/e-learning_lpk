...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8" 
    x-data="quizBuilder(@js($quiz->questions->map(function($q) { 
        return [
    ...
    <nav class="flex items-center space-x-2 text-sm mb-6">
        ...
        <span class="text-[#d4af37] font-medium">Edit Quiz</span>
    </nav>
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-white tracking-wide">Edit Quiz</h1>
            <p class="text-sm text-gray-400">{{ $quiz->title }}</p>
        </div>
    </div>

    {{-- Import Section (OUTSIDE main form to prevent nested form issue) --}}
    <div class="bg-gradient-to-b from-[#0f172a] to-[#0b1221] border border-green-500/30 rounded-2xl shadow-2xl overflow-hidden mb-6">
        ...
        <div class="p-6">
            ...
            <form action="{{ route('instructor.quizzes.import', $quiz) }}" method="POST" enctype="multipart/form-data" class="flex flex-wrap gap-4 items-center">
                @csrf
                <div x-data="{ fileName: '', hasFile: false }" class="flex items-center gap-3">
                    <label class="inline-flex items-center px-4 py-2 rounded-xl border border-[#1e293b] text-gray-300 hover:text-[#d4af37] hover:border-[#d4af37] transition-all text-sm font-bold cursor-pointer">
                        ...
                        Pilih File Excel
                        <input type="file" name="file" accept=".xlsx,.xls" class="hidden" required
                    ...
                    <button type="submit" x-show="hasFile"
                        class="inline-flex items-center px-4 py-2 rounded-xl bg-green-600 text-white hover:bg-green-700 transition-all text-sm font-bold">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <p class="text-xs text-gray-500 mt-3">
                ...
                Soal yang diimport akan ditambahkan ke daftar soal yang sudah ada. Refresh halaman setelah import.
            </p>
        </div>
    </div>

    {{-- Form --}}
    <form action="{{ route('instructor.quizzes.update', $quiz) }}" method="POST" enctype="multipart/form-data" @submit="validateForm">
        @csrf
        @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="duration_minutes" value="{{ old('duration_minutes', $quiz->duration_minutes) }}" required min="1" max="300"
                            class="w-full bg-[#0b1221] border border-[#1e293b] text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#d4af37]/50 focus:border-[#d4af37] transition-all">
                    </div>
                    <div>
                        <label class="block text-[#d4af37] text-xs font-bold uppercase mb-2 tracking-wider">
                ...
                <div class="pt-6 border-t border-[#1e293b]">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {{-- Access Code Toggle --}}
                        <div x-data="{ useCode: {{ $quiz->access_code ? 'true' : 'false' }} }">
                            <label class="flex items-center gap-3 cursor-pointer mb-2">
                                <input type="checkbox" name="use_access_code" value="1" x-model="useCode"
                                    {{ old('use_access_code', $quiz->access_code ? true : false) ? 'checked' : '' }}
                        ...
                        <div>
                            <label class="flex items-center gap-3 cursor-pointer mb-2">
                                <input type="checkbox" name="show_answers_after" value="1"
                                    {{ old('show_answers_after', $quiz->show_answers_after) ? 'checked' : '' }}
                                    class="w-5 h-5 rounded border-[#1e293b] bg-[#0b1221] text-[#d4af37] focus:ring-[#d4af37]/50">
